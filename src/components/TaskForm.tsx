...
export function TaskForm({
  ...
  useEffect(() => {
    if (isOpen) {
      if (taskToEdit) {
        const dueDateObj = taskToEdit.dueDate
    ...
    const fetchMembers = async (teamIds: string[]) => {
      ...
      try {
        const memberPromises = teamIds.map((id) =>
          fetch(`/api/teams/${id}/members`).then((res) =>
            res.ok ? res.json() : []
          )
        );
        ...
        const uniqueMembers = Array.from(
          new Map(
            allMembers.map((item: TeamMember) => [item.id, item])
          ).values()
        );
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {taskToEdit
              ...
              : "Fill in the details for your new task to keep things organized."}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        {...field}
                        className="bg-background border-input focus:ring-primary text-base"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
