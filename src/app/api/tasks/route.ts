  ...
  try {
    ...
    const formattedTasks = tasks.map((task) => {
      ...
      return {
        ...
        assignedTo: assignedToData?.map((a) => ({
          id: a._id.toString(),
          name: a.name,
          email: a.email,
        })),
...
export async function POST(req: Request) {
  ...
  try {
    await dbConnect();
    const { title, notes, priority, teamIds, assignedTo, dueDate } =
      await req.json();
    ...
    if (task.assignedTo && task.dueDate) {
      const assigneesToNotify = task.assignedTo.filter(
      ...
      if (assigneesToNotify.length > 0) {
        const notifications = assigneesToNotify.map((userId) => ({
          userId: userId,
          type: "TASK_ASSIGNED",
          message: `${session.user.name} assigned you the task: "${
            task.title
          }", due on ${format(task.dueDate!, "PPP 'at' p")}`,
